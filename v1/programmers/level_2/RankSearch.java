 ...
 * 중요한 점
 ...
 - 정렬된 이후, 각 List를 탐색하는 과정에서 일정 값 이상의 경우에 대한 개수만 알면 되니 이진 탐색으로 효율성을 높힌다.
...
public class RankSearch {
  ...
  public int[] solution(String[] info, String[] query) {
    ...
    for (int i=0; i< query.length; i++) {
      ...
      // Collections.binarySearch(list, score); -> Collections 클래스에서 제공해주는 이진 탐색 메소드
  ...
  public int binarySearch(List<Integer> list, int score) {
    ...
    while(low <= high) {
      ...
      if (midScore < score) {
        ...
        result = mid; // 큰 값이 나온 경우 해당 인덱스를 수시로 업데이트
        high = mid - 1;
      }
    }
    return result;
  ...
  // 최소값 구하기
  private int binarySearch(int[] scoville, int K) {
    int low = 0;
    int high = scoville.length - 1;
    int result = 0;
    ...
    while(low <= high) {
      ...
      if (scoville[mid] < K) {
        result = mid; // 작은 경우 해당 인덱스를 수시로 업데이트
        low = mid + 1;
      }
      else if(scoville[mid] > K) {
        high = mid - 1;
      }
      else return mid;
    }
